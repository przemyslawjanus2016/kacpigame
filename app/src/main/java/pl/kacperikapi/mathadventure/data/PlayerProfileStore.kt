...
/**
 * Lokalne profile graczy. Dane i postęp pozostają wyłącznie na urządzeniu.
 */
class PlayerProfileStore(context: Context) {
    ...
    fun activeProfile(): PlayerProfile? =
        activeProfileId()?.let { id -> profiles().firstOrNull { it.id == id } }
    ...
    fun add(name: String): PlayerProfile {
        ...
        require(safeName.isNotBlank()) { "Nazwa profilu nie może być pusta" }
    ...
    fun delete(profileId: String) {
        ...
        appContext.getSharedPreferences(progressPrefsName(profileId), Context.MODE_PRIVATE)
            .edit()
            .clear()
            .apply()
    ...
    /**
     * Zachowuje postęp ze starszych wersji z jednym globalnym zapisem.
     */
    ...
    private fun sanitizeName(raw: String): String =
        raw.trim().replace(Regex("\\s+"), " ").take(24)
    ...
    companion object {
        ...
        private val LEGACY_PROGRESS_KEYS = setOf(
            ...
            "stage_stars", "solved", "correct", "daily_streak", "daily_last_date",
            "daily_total", "postcards"
        )

        fun progressPrefsName(profileId: String): String =
            "kacper_kapi_progress_$profileId"
    }
}
